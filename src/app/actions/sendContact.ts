...
export interface ContactData {
  fullName: string;
  email: string;
  category: string; // 'Soru', 'İş Birliği', 'Etkinlik / Konuşmacı', 'Diğer'
  message: string;
}
...
export async function sendContactAction(data: ContactData) {
  ...
  if (!rateCheck.allowed) {
    return {
      success: false,
      error: `Çok fazla mesaj denemesi yaptınız. Lütfen ${rateCheck.retryAfterSeconds} saniye bekledikten sonra tekrar deneyiniz.`
    };
  }

  // 1. Save local record in content/contact/
  try {
    const dirPath = path.join(process.cwd(), 'content', 'contact');
    ...
    const filename = `${timestamp}-${safeName || 'iletisim'}.json`;
    const record = {
      targetEmail,
...
==================================================`;
  ...
  if (cleanPass) {
    try {
      ...
      console.log(`✅ İletişim e-postası ${targetEmail} adresine başarıyla gönderildi.`);
